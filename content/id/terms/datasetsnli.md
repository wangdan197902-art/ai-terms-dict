---
title: "Dataset:Snli"
term_id: "datasetsnli"
category: "basic_concepts"
subcategory: ""
tags: ["NLI", "Stanford", "Benchmark", "Semantics"]
difficulty: 2
weight: 1
slug: "datasetsnli"
date: "2026-07-18T15:46:34.921597Z"
lastmod: "2026-07-18T16:38:07.447062Z"
draft: false
source: "agnes_llm"
status: "published"
language: "id"
description: "Korpus Inferensi Bahasa Alami Stanford, sebuah dataset besar berisi kalimat-kalimat bahasa Inggris yang dipasangkan dengan label inkonsistensi teks yang ditulis oleh manusia."
---
## Definition

SNLI adalah dataset benchmark yang berisi lebih dari 500.000 pasangan kalimat berlabel yang dianotasi dengan tiga kelas: inkonsistensi (entailment), kontradiksi, dan netral. Dataset ini dibuat untuk memajukan penelitian dalam...

### Summary

Korpus Inferensi Bahasa Alami Stanford, sebuah dataset besar berisi kalimat-kalimat bahasa Inggris yang dipasangkan dengan label inkonsistensi teks yang ditulis oleh manusia.

## Key Concepts

- Inferensi Bahasa Alami
- Inkonsistensi Teks
- Pasangan Kalimat
- Benchmark

## Use Cases

- Mengevaluasi model kesamaan semantik
- Melatih klasifikator NLI
- Meneliti penalaran logis dalam Pemrosesan Bahasa Alami

## Related Terms

- [MultiNLI](/en/terms/multinli/)
- [RTE (Recognizing Textual Entailment)](/en/terms/rte-recognizing-textual-entailment/)
- [BERT](/en/terms/bert/)
- [Pelabelan Peran Semantik](/en/terms/pelabelan-peran-semantik/)
