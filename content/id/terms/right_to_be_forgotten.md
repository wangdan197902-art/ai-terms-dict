---
title: "Hak untuk Dilupakan"
term_id: "right_to_be_forgotten"
category: "ethics_safety"
subcategory: ""
tags: ["privacy", "legal"]
difficulty: 3
weight: 1
slug: "right_to_be_forgotten"
date: "2026-07-18T16:14:07.303242Z"
lastmod: "2026-07-18T16:38:07.521422Z"
draft: false
source: "agnes_llm"
status: "published"
language: "id"
description: "Konsep hukum yang memungkinkan individu meminta penghapusan data pribadi mereka yang disimpan oleh organisasi."
---
## Definition

Hak untuk dilupakan memungkinkan pengguna menuntut penghapusan informasi pribadi mereka dari basis data dan set pelatihan AI. Mengimplementasikan hal ini dalam pembelajaran mesin sangat menantang karena model mungkin telah menginternalisasikan pola dari data tersebut, sehingga memerlukan teknik seperti pembelajaran mesin ulang (machine unlearning) untuk melupakan informasi spesifik tanpa melatih ulang model dari awal.

### Summary

Konsep hukum yang memungkinkan individu meminta penghapusan data pribadi mereka yang disimpan oleh organisasi.

## Key Concepts

- Penghapusan Data
- Machine Unlearning (Pelatihan Ulang Mesin)
- Kepatuhan Hukum
- Pelatihan Ulang Model

## Use Cases

- Mematuhi permintaan penghapusan GDPR
- Menghapus data historis yang bias atau berbahaya dari model
- Mengelola pencabutan persetujuan pengguna dalam layanan AI

## Related Terms

- [Machine Unlearning (Pelatihan Ulang Mesin)](/en/terms/machine-unlearning-pelatihan-ulang-mesin/)
- [GDPR (Peraturan Perlindungan Data Umum UE)](/en/terms/gdpr-peraturan-perlindungan-data-umum-ue/)
- [Data Erasure (Penghapusan Data)](/en/terms/data-erasure-penghapusan-data/)
- [Consent Management (Manajemen Persetujuan)](/en/terms/consent-management-manajemen-persetujuan/)
