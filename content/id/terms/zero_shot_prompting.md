---
title: "Prompting Zero-Shot"
term_id: "zero_shot_prompting"
category: "application_paradigms"
subcategory: ""
tags: ["Prompting", "LLM Applications", "NLP"]
difficulty: 2
weight: 1
slug: "zero_shot_prompting"
date: "2026-07-18T16:13:46.637860Z"
lastmod: "2026-07-18T16:38:07.520663Z"
draft: false
source: "agnes_llm"
status: "published"
language: "id"
description: "Sebuah teknik di mana model bahasa besar melakukan tugas tanpa contoh sebelumnya atau penyetelan halus, hanya mengandalkan instruksi bahasa alami."
---
## Definition

Prompting zero-shot melibatkan permintaan kepada model bahasa yang telah dilatih sebelumnya untuk menyelesaikan tugas secara langsung melalui prompt teks, tanpa memberikan contoh few-shot atau melakukan penyetelan tambahan. Mod...

### Summary

Sebuah teknik di mana model bahasa besar melakukan tugas tanpa contoh sebelumnya atau penyetelan halus, hanya mengandalkan instruksi bahasa alami.

## Key Concepts

- Rekayasa Prompt
- Kemampuan Emergen
- Pembelajaran Konteks-In
- Penyetelan Instruksi

## Use Cases

- Prototipe cepat aplikasi AI
- Pergantian tugas dinamis
- Mengurangi biaya anotasi data

## Related Terms

- [Few-Shot Prompting (Prompting dengan Beberapa Contoh)](/en/terms/few-shot-prompting-prompting-dengan-beberapa-contoh/)
- [Chain-of-Thought (Rantai Pemikiran)](/en/terms/chain-of-thought-rantai-pemikiran/)
- [Fine-Tuning (Penyetelan Halus)](/en/terms/fine-tuning-penyetelan-halus/)
- [LLM (Large Language Model / Model Bahasa Besar)](/en/terms/llm-large-language-model-model-bahasa-besar/)
