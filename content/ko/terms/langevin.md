---
title: "랑주뱅"
term_id: "langevin"
category: "basic_concepts"
subcategory: ""
tags: ["mathematics", "sampling"]
difficulty: 4
weight: 1
slug: "langevin"
date: "2026-07-18T15:25:30.630901Z"
lastmod: "2026-07-18T16:38:06.776934Z"
draft: false
source: "agnes_llm"
status: "published"
language: "ko"
description: "랑주뱅은 확률 미분 방정식을 참조하며, 특히 마찰과 노이즈가 있는 물리적 운동을 시뮬레이션하여 확률 분포에서 샘플링하기 위해 사용되는 랑주뱅 동역학을 의미합니다."
---
## Definition

랑주뱅 동역학은 에너지 경관을 효율적으로 탐색하기 위해 무작위 노이즈와 감쇠력을 통합합니다. AI에서는 주로 해밀턴 몬테 카를로나 확률적 그래디언트와 같은 샘플링 방법에 사용됩니다.

### Summary

랑주뱅은 확률 미분 방정식을 참조하며, 특히 마찰과 노이즈가 있는 물리적 운동을 시뮬레이션하여 확률 분포에서 샘플링하기 위해 사용되는 랑주뱅 동역학을 의미합니다.

## Key Concepts

- 확률 미분 방정식
- 샘플링
- 베이지안 추론
- 노이즈 주입

## Use Cases

- 베이지안 신경망
- MCMC 샘플링
- 고차원 공간에서의 최적화

## Related Terms

- [Monte Carlo (몬테카를로)](/en/terms/monte-carlo-몬테카를로/)
- [stochastic (확률적)](/en/terms/stochastic-확률적/)
- [dynamics (동역학)](/en/terms/dynamics-동역학/)
- [inference (추론)](/en/terms/inference-추론/)
