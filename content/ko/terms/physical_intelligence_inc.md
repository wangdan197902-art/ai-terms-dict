---
title: 피지컬 인텔리전스(Physical Intelligence Inc.)
term_id: physical_intelligence_inc
category: basic_concepts
subcategory: ''
tags:
- robotics
- companies
- Embodied AI
difficulty: 3
weight: 1
slug: physical_intelligence_inc
date: '2026-07-18T16:10:11.698233Z'
lastmod: '2026-07-18T16:38:06.896760Z'
draft: false
source: agnes_llm
status: published
language: ko
description: 일반 목적 로봇 조작에 중점을 둔 전 딥마인드 연구원들이 설립한 로봇공학 기업입니다.
---
## Definition

피지컬 인텔리전스(PI)는 구글 딥마인드에서 분사된 기업으로, 신체화된 인공지능(Embodied AI) 및 로봇공학 분야를 발전시키는 것을 목표로 합니다. 이 회사는 다양한 작업을 수행할 수 있는 범용 로봇 개발에 집중하고 있습니다.

### Summary

일반 목적 로봇 조작에 중점을 둔 전 딥마인드 연구원들이 설립한 로봇공학 기업입니다.

## Key Concepts

- 신체화된 인공지능
- 로봇공학
- 조작(Manipulation)
- 딥마인드 분사 기업

## Use Cases

- 자동화 창고 물류
- 복합 조립 라인 작업
- 범용 로봇 에이전트 연구

## Related Terms

- [Robot Learning (로봇 학습)](/en/terms/robot-learning-로봇-학습/)
- [Sim-to-Real Transfer (시뮬레이션에서 현실로의 전이)](/en/terms/sim-to-real-transfer-시뮬레이션에서-현실로의-전이/)
- [Computer Vision (컴퓨터 비전)](/en/terms/computer-vision-컴퓨터-비전/)
- [Actuators (구동기)](/en/terms/actuators-구동기/)
