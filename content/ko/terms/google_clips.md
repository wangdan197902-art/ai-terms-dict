---
title: "구글 클립스 (Google Clips)"
term_id: "google_clips"
category: "engineering_practice"
subcategory: ""
tags: ["hardware", "computer_vision", "discontinued"]
difficulty: 2
weight: 1
slug: "google_clips"
aliases:
  - /ko/terms/google_clips/
date: "2026-07-18T15:57:27.032218Z"
lastmod: "2026-07-18T16:38:06.846021Z"
draft: false
source: "agnes_llm"
status: "published"
language: "ko"
description: "머신러닝을 활용하여 자동으로 순간을 포착하도록 설계된 주머니 크기 AI 카메라입니다."
---

## Definition

구글 클립스는 구글이 개발한 소비자 전자 제품으로, 기기 내 머신러닝을 사용하여 얼굴이나 애완동물 등 흥미로운 장면과 피사체를 식별하고 자동으로 사진을 촬영했습니다.

### Summary

머신러닝을 활용하여 자동으로 순간을 포착하도록 설계된 주머니 크기 AI 카메라입니다.

## Key Concepts

- 컴퓨터 비전
- 엣지 AI (단말기 인공지능)
- 자동 사진 촬영
- 사물인터넷 (IoT)

## Use Cases

- 캐주얼 사진 촬영
- 반려동물 모니터링
- 가족의 추억 기록

## Related Terms

- [스마트 카메라](/en/terms/스마트-카메라/)
- [객체 감지](/en/terms/객체-감지/)
- [TensorFlow Lite (텐서플로우 라이트)](/en/terms/tensorflow-lite-텐서플로우-라이트/)
