---
title: "모시(Moshi)"
term_id: "moshi"
category: "basic_concepts"
subcategory: ""
tags: ["Multimodal", "Speech", "Research"]
difficulty: 4
weight: 1
slug: "moshi"
aliases:
  - /ko/terms/moshi/
date: "2026-07-18T16:06:20.712606Z"
lastmod: "2026-07-18T16:38:06.886925Z"
draft: false
source: "agnes_llm"
status: "published"
language: "ko"
description: "Kyutai가 개발한 음성-언어 모델로, 원활한 다중 모달 상호작용을 위해 텍스트와 오디오 표현을 함께 학습합니다."
---

## Definition

모시는 Kyutai가 만든 고급 AI 모델로, 음성 처리와 텍스트 처리를 통합된 프레임워크로 결합합니다. 전통적인 시스템이 음성을 텍스트로 변환한 후 처리하는 것과 달리, 모시는 음성 및 텍스트 표현을 함께 학습하여 자연스러운 상호작용을 가능하게 합니다.

### Summary

Kyutai가 개발한 음성-언어 모델로, 원활한 다중 모달 상호작용을 위해 텍스트와 오디오 표현을 함께 학습합니다.

## Key Concepts

- 다중 모달 학습
- 음성-텍스트 공동 모델링
- 운율 보존
- 실시간 상호작용

## Use Cases

- 자연스러운 음성 비서 구축
- 감정 톤을 활용한 대화형 스토리텔링 강화
- 청각 장애인용 접근성 도구 개선

## Related Terms

- [Kyutai (모델 개발사)](/en/terms/kyutai-모델-개발사/)
- [다중 모달 AI (여러 데이터 유형을 처리하는 AI)](/en/terms/다중-모달-ai-여러-데이터-유형을-처리하는-ai/)
- [음성 인식 (음성을 텍스트로 변환하는 기술)](/en/terms/음성-인식-음성을-텍스트로-변환하는-기술/)
- [대화형 AI (사용자와 대화하는 인공지능)](/en/terms/대화형-ai-사용자와-대화하는-인공지능/)
