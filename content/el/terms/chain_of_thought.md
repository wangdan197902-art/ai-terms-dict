---
title: Αλυσιδωτή Σκέψη
term_id: chain_of_thought
category: training_techniques
subcategory: ''
tags:
- prompting
- reasoning
- LLM
difficulty: 3
weight: 1
slug: chain_of_thought
date: '2026-07-18T15:22:38.434889Z'
lastmod: '2026-07-18T17:15:09.838238Z'
draft: false
source: agnes_llm
status: published
language: el
description: Μια τεχνική prompting που ενθαρρύνει τα μεγάλα γλωσσικά μοντέλα (LLMs)
  να παράγουν ενδιάμεσα βήματα λογικής πριν απαντήσουν.
---
## Definition

Η προτροπή αλυσιδωτής σκέψης (Chain-of-Thought - CoT) είναι μια στρατηγική όπου τα μεγάλα γλωσσικά μοντέλα κατευθύνονται να παράγουν επεξηγήσεις λογικής βήμα προς βήμα πριν καταλήξουν σε μια τελική απάντηση. Διασπώντας σύνθετα προβλήματα

### Summary

Μια τεχνική prompting που ενθαρρύνει τα μεγάλα γλωσσικά μοντέλα (LLMs) να παράγουν ενδιάμεσα βήματα λογικής πριν απαντήσουν.

## Key Concepts

- Μηχανική Προτροπής (Prompt Engineering)
- Βήματα Λογικής (Reasoning Steps)
- Ερμηνευσιμότητα (Interpretability)
- Zero-Shot CoT (Αλυσιδωτή Σκέψη Μηδενικής Εκπαίδευσης)

## Use Cases

- Επίλυση σύνθετων μαθηματικών προβλημάτων με λέξεις
- Εργασίες λογικής απαγωγής
- Αποσφαλμάτωση αποσπασμάτων κώδικα

## Related Terms

- [Prompt Engineering (Μηχανική Προτροπής)](/en/terms/prompt-engineering-μηχανική-προτροπής/)
- [Large Language Models (Μεγάλα Γλωσσικά Μοντέλα)](/en/terms/large-language-models-μεγάλα-γλωσσικά-μοντέλα/)
- [Few-Shot Learning (Μάθηση Λιγών Παραδειγμάτων)](/en/terms/few-shot-learning-μάθηση-λιγών-παραδειγμάτων/)
