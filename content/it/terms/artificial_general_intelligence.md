---
title: "Intelligenza generale artificiale"
term_id: "artificial_general_intelligence"
category: "basic_concepts"
subcategory: ""
tags: ["future_tech", "theory", "intelligence"]
difficulty: 5
weight: 1
slug: "artificial_general_intelligence"
aliases:
  - /it/terms/artificial_general_intelligence/
date: "2026-07-18T15:47:37.461697Z"
lastmod: "2026-07-18T17:15:08.598626Z"
draft: false
source: "agnes_llm"
status: "published"
language: "it"
description: "IA ipotetica dotata della capacità di comprendere, apprendere e applicare conoscenze in una vasta gamma di compiti a livello umano."
---

## Definition

L'Intelligenza Generale Artificiale (AGI) si riferisce a un tipo di IA in grado di eseguire qualsiasi compito intellettuale che un essere umano può svolgere. A differenza dell'IA stretta, che eccelle in compiti specifici come gli scacchi o il riconoscimento...

### Summary

IA ipotetica dotata della capacità di comprendere, apprendere e applicare conoscenze in una vasta gamma di compiti a livello umano.

## Key Concepts

- Intelligenza a livello umano
- Apprendimento per trasferimento
- Ragionamento del senso comune
- Adattamento autonomo

## Use Cases

- Assistenti personali universali capaci di pianificazione complessa
- Scoperta scientifica attraverso più discipline
- Sistemi educativi adattivi su misura per le esigenze individuali

## Related Terms

- [narrow_ai (intelligenza artificiale stretta)](/en/terms/narrow_ai-intelligenza-artificiale-stretta/)
- [superintelligence (superintelligenza)](/en/terms/superintelligence-superintelligenza/)
- [machine_learning (apprendimento automatico)](/en/terms/machine_learning-apprendimento-automatico/)
- [cognitive_architecture (architettura cognitiva)](/en/terms/cognitive_architecture-architettura-cognitiva/)
