---
title: THUDM
term_id: thudm
category: basic_concepts
subcategory: ''
tags:
- organizations
- Open Source
difficulty: 2
weight: 1
slug: thudm
date: '2026-07-18T11:10:28.539942Z'
lastmod: '2026-07-18T11:44:44.862005Z'
draft: false
source: agnes_llm
status: published
language: es
description: THUDM es un grupo de investigación de la Universidad de Tsinghua conocido
  por desarrollar modelos de IA de código abierto como ChatGLM.
---
## Definition

THUDM (Grupo de Investigación de Procesamiento de Lenguaje Natural de la Universidad de Tsinghua) es una entidad académica e investigadora destacada centrada en la inteligencia artificial, particularmente en el procesamiento del lenguaje natural. Han...

### Summary

THUDM es un grupo de investigación de la Universidad de Tsinghua conocido por desarrollar modelos de IA de código abierto como ChatGLM.

## Key Concepts

- IA de Código Abierto
- ChatGLM
- Investigación de PLN
- Universidad de Tsinghua

## Use Cases

- Acceso a LLMs de Código Abierto
- Referencia para Investigación Académica
- Desarrollo de Aplicaciones Personalizadas de PLN

## Related Terms

- [ChatGLM (ChatGLM)](/en/terms/chatglm-chatglm/)
- [Large Language Models (Modelos de Lenguaje Grande)](/en/terms/large-language-models-modelos-de-lenguaje-grande/)
- [NLP (PLN)](/en/terms/nlp-pln/)
- [Open Source (Código Abierto)](/en/terms/open-source-código-abierto/)
