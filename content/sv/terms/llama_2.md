---
title: "Llama 2"
term_id: "llama_2"
category: "basic_concepts"
subcategory: ""
tags: ["LLM", "Meta", "Open Source"]
difficulty: 2
weight: 1
slug: "llama_2"
aliases:
  - /sv/terms/llama_2/
date: "2026-07-18T16:07:28.128200Z"
lastmod: "2026-07-18T17:15:09.021730Z"
draft: false
source: "agnes_llm"
status: "published"
language: "sv"
description: "Llama 2 är en familj av stora språkmodeller som släpptes av Meta i juli 2023, med optimerade arkitekturer och större kontextfönster jämfört med dess föregångare."
---

## Definition

Släppt av Meta AI i juli 2023, representerar Llama 2 en betydande utveckling inom öppna språkmodeller med tillgängliga vikter. Den erbjuder förtränade och finjusterade varianter som sträcker sig från 7 miljarder till 70 miljarder parametrar.

### Summary

Llama 2 är en familj av stora språkmodeller som släpptes av Meta i juli 2023, med optimerade arkitekturer och större kontextfönster jämfört med dess föregångare.

## Key Concepts

- Öppna modeller med tillgängliga vikter
- Transformerarkitektur
- Kontextfönster
- Finjustering

## Use Cases

- Prototyputveckling inom forskning
- Utveckling av anpassade chattbottar
- Kodgenereringsuppgifter

## Related Terms

- [LLaMA](/en/terms/llama/)
- [Meta AI](/en/terms/meta-ai/)
- [Öppen källkod LLMs (Stora språkmodeller med öppen källkod)](/en/terms/öppen-källkod-llms-stora-språkmodeller-med-öppen-källkod/)
- [Hugging Face](/en/terms/hugging-face/)
