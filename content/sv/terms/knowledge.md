---
title: "Kunskap"
term_id: "knowledge"
category: "basic_concepts"
subcategory: ""
tags: ["fundamentals", "cognitive"]
difficulty: 2
weight: 1
slug: "knowledge"
date: "2026-07-18T15:27:04.076414Z"
lastmod: "2026-07-18T17:15:08.944552Z"
draft: false
source: "agnes_llm"
status: "published"
language: "sv"
description: "Kunskap är den strukturerade förståelsen, fakta, färdigheter och insikter som härleds från information, erfarenhet eller resonemang, vilket möjliggör effektivt beslutsfattande."
---
## Definition

Inom AI syftar kunskap ofta på explicit information som lagras i databaser, ontologier eller neuronnätverksvikter, vilket möjliggör resonemang och inferens. Den befinner sig ovanför information i DIKW-hierarkin (Data, Information, Knowledge, Wisdom)...

### Summary

Kunskap är den strukturerade förståelsen, fakta, färdigheter och insikter som härleds från information, erfarenhet eller resonemang, vilket möjliggör effektivt beslutsfattande.

## Key Concepts

- DIKW-hierarkin
- Resonemang
- Implicit kontra explicit
- Faktahämtning

## Use Cases

- Fråge-och-svars-system
- Expertssystem
- RAG (Retrieval-Augmented Generation)

## Related Terms

- [information (bearbetad data)](/en/terms/information-bearbetad-data/)
- [resonemang (slutledning)](/en/terms/resonemang-slutledning/)
- [minne (lagring av data)](/en/terms/minne-lagring-av-data/)
- [fakta ( verifierbara uppgifter)](/en/terms/fakta-verifierbara-uppgifter/)
