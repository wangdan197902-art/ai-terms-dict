---
title: THUDM
term_id: thudm
category: basic_concepts
subcategory: ''
tags:
- organizations
- Open Source
difficulty: 2
weight: 1
slug: thudm
date: '2026-07-18T16:14:42.225338Z'
lastmod: '2026-07-18T16:38:07.812364Z'
draft: false
source: agnes_llm
status: published
language: vi
description: THUDM là một nhóm nghiên cứu tại Đại học Thanh Hoa, nổi tiếng với việc
  phát triển các mô hình AI mã nguồn mở như ChatGLM.
---
## Definition

THUDM (Nhóm nghiên cứu Xử lý Ngôn ngữ Tự nhiên thuộc Đại học Thanh Hoa) là một tổ chức học thuật và nghiên cứu nổi bật tập trung vào trí tuệ nhân tạo, đặc biệt là xử lý ngôn ngữ tự nhiên. Họ...

### Summary

THUDM là một nhóm nghiên cứu tại Đại học Thanh Hoa, nổi tiếng với việc phát triển các mô hình AI mã nguồn mở như ChatGLM.

## Key Concepts

- AI mã nguồn mở
- ChatGLM
- Nghiên cứu NLP
- Đại học Thanh Hoa

## Use Cases

- Truy cập các LLM mã nguồn mở
- Tham chiếu nghiên cứu học thuật
- Xây dựng ứng dụng NLP tùy chỉnh

## Related Terms

- [ChatGLM](/en/terms/chatglm/)
- [Large Language Models (Mô hình ngôn ngữ lớn)](/en/terms/large-language-models-mô-hình-ngôn-ngữ-lớn/)
- [NLP (Xử lý ngôn ngữ tự nhiên)](/en/terms/nlp-xử-lý-ngôn-ngữ-tự-nhiên/)
- [Open Source (Mã nguồn mở)](/en/terms/open-source-mã-nguồn-mở/)
