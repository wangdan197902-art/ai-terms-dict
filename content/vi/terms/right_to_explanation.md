---
title: "Quyền được giải thích"
term_id: "right_to_explanation"
category: "basic_concepts"
subcategory: ""
tags: ["ethics", "legal", "xai"]
difficulty: 2
weight: 1
slug: "right_to_explanation"
date: "2026-07-18T16:10:43.773848Z"
lastmod: "2026-07-18T16:38:07.801547Z"
draft: false
source: "agnes_llm"
status: "published"
language: "vi"
description: "Một nguyên tắc pháp lý và đạo đức trao cho cá nhân quyền nhận được thông tin có ý nghĩa về các quyết định tự động hóa ảnh hưởng đáng kể đến họ."
---
## Definition

Quyền được giải thích là một thành phần cốt lõi của trách nhiệm giải trình thuật toán, đặc biệt trong các khung pháp lý như GDPR. Nó đảm bảo rằng khi hệ thống AI đưa ra quyết định tác động đến quyền lợi của một người

### Summary

Một nguyên tắc pháp lý và đạo đức trao cho cá nhân quyền nhận được thông tin có ý nghĩa về các quyết định tự động hóa ảnh hưởng đáng kể đến họ.

## Key Concepts

- Minh bạch thuật toán
- Tuân thủ GDPR
- AI có thể giải thích (XAI)
- Trách nhiệm giải trình

## Use Cases

- Hệ thống chấm điểm tín dụng
- Nền tảng tuyển dụng tự động
- Công cụ đánh giá rủi ro tư pháp

## Related Terms

- [explainable_ai (AI có thể giải thích)](/en/terms/explainable_ai-ai-có-thể-giải-thích/)
- [algorithmic_bias (thiên kiến thuật toán)](/en/terms/algorithmic_bias-thiên-kiến-thuật-toán/)
- [data_privacy (quyền riêng tư dữ liệu)](/en/terms/data_privacy-quyền-riêng-tư-dữ-liệu/)
- [transparency (sự minh bạch)](/en/terms/transparency-sự-minh-bạch/)
