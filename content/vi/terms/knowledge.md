---
title: "Tri thức"
term_id: "knowledge"
category: "basic_concepts"
subcategory: ""
tags: ["fundamentals", "cognitive"]
difficulty: 2
weight: 1
slug: "knowledge"
aliases:
  - /vi/terms/knowledge/
date: "2026-07-18T15:26:22.522702Z"
lastmod: "2026-07-18T16:38:07.688114Z"
draft: false
source: "agnes_llm"
status: "published"
language: "vi"
description: "Tri thức là sự hiểu biết có cấu trúc, các sự thật, kỹ năng và thông tin chi tiết thu được từ thông tin, kinh nghiệm hoặc lập luận, giúp đưa ra quyết định hiệu quả."
---

## Definition

Trong AI, tri thức thường đề cập đến thông tin rõ ràng được lưu trữ trong cơ sở dữ liệu, ontology hoặc trọng số mạng nơ-ron cho phép suy luận. Nó nằm ở vị trí cao hơn thông tin trong phân cấp DIKW

### Summary

Tri thức là sự hiểu biết có cấu trúc, các sự thật, kỹ năng và thông tin chi tiết thu được từ thông tin, kinh nghiệm hoặc lập luận, giúp đưa ra quyết định hiệu quả.

## Key Concepts

- Phân cấp DIKW
- Suy luận
- Ẩn vs Rõ ràng
- Truy xuất Sự kiện

## Use Cases

- Hệ thống hỏi đáp
- Hệ thống chuyên gia
- RAG (Sinh có tăng cường truy xuất)

## Related Terms

- [information (thông tin)](/en/terms/information-thông-tin/)
- [reasoning (suy luận)](/en/terms/reasoning-suy-luận/)
- [memory (bộ nhớ)](/en/terms/memory-bộ-nhớ/)
- [facts (sự thật/sự kiện)](/en/terms/facts-sự-thật-sự-kiện/)
