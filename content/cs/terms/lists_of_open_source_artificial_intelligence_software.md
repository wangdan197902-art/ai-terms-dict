---
title: "Seznamy softwaru pro umělou inteligenci s otevřeným zdrojovým kódem"
term_id: "lists_of_open_source_artificial_intelligence_software"
category: "basic_concepts"
subcategory: ""
tags: ["resources", "community", "open_source"]
difficulty: 1
weight: 1
slug: "lists_of_open_source_artificial_intelligence_software"
aliases:
  - /cs/terms/lists_of_open_source_artificial_intelligence_software/
date: "2026-07-18T16:05:59.848049Z"
lastmod: "2026-07-18T17:15:09.148626Z"
draft: false
source: "agnes_llm"
status: "published"
language: "cs"
description: "Kurátorované sbírky nebo adresáře katalogizující veřejně dostupné nástroje, knihovny a frameworky AI."
---

## Definition

Tyto seznamy zahrnují organizované repozitáře, jako jsou témata na GitHubu, seznamy typu „Awesome“ nebo komunitou udržované wiki, které agregují software s otevřeným zdrojovým kódem související s umělou inteligencí. Slouží jako zásadní zdroje pro objevování technologií.

### Summary

Kurátorované sbírky nebo adresáře katalogizující veřejně dostupné nástroje, knihovny a frameworky AI.

## Key Concepts

- Otevřený zdroj
- Kurátorství zdrojů
- Komunitní údržba
- Objevování softwaru

## Use Cases

- Objevování výzkumných nástrojů
- Výběr technologického stacku pro startupy
- Shromažďování vzdělávacích zdrojů

## Related Terms

- [awesome_list (seznamy typu Awesome)](/en/terms/awesome_list-seznamy-typu-awesome/)
- [github_topics (témata na GitHubu)](/en/terms/github_topics-témata-na-githubu/)
- [open_source_ecosystem (ekosystém s otevřeným zdrojovým kódem)](/en/terms/open_source_ecosystem-ekosystém-s-otevřeným-zdrojovým-kódem/)
- [ai_frameworks (frameworky pro AI)](/en/terms/ai_frameworks-frameworky-pro-ai/)
