---
title: "Sociálně asistivní robot"
term_id: "socially_assistive_robot"
category: "basic_concepts"
subcategory: ""
tags: ["robotics", "healthcare", "hri"]
difficulty: 3
weight: 1
slug: "socially_assistive_robot"
aliases:
  - /cs/terms/socially_assistive_robot/
date: "2026-07-18T16:17:54.808058Z"
lastmod: "2026-07-18T17:15:09.201370Z"
draft: false
source: "agnes_llm"
status: "published"
language: "cs"
description: "Robot navržený k interakci s lidmi za účelem zlepšení jejich fyzického nebo psychického blaha bez vykonávání fyzických úkolů."
---

## Definition

Sociálně asistivní roboti (SAR) jsou podskupinou lidsko-robotické interakce zaměřenou na poskytování pomoci prostřednictvím sociálních mechanismů spíše než fyzické manipulace. Využívají strategie bez fyzického kontaktu, jako je verbální komunikace, neverbální signály a emocionální odrazy, aby podpořily nezávislost uživatelů, zejména u starších osob nebo lidí se zdravotním postižením, a zlepšili jejich kvalitu života.

### Summary

Robot navržený k interakci s lidmi za účelem zlepšení jejich fyzického nebo psychického blaha bez vykonávání fyzických úkolů.

## Key Concepts

- Lidsko-robotická interakce (HRI)
- Asistence bez fyzického kontaktu
- Emoční výpočty
- Rehabilitace

## Use Cases

- Společníci pro seniory
- Podpora terapie autismu
- Motivace při fyzioterapii

## Related Terms

- [Péče robot (Care Robot)](/en/terms/péče-robot-care-robot/)
- [Společenský robot (Companion Robot)](/en/terms/společenský-robot-companion-robot/)
- [Afektivní výpočty (Affective Computing)](/en/terms/afektivní-výpočty-affective-computing/)
- [Lidsko-robotická interakce (HRI)](/en/terms/lidsko-robotická-interakce-hri/)
