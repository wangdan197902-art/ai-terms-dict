---
title: "Data Science and Predictive Analytics"
term_id: "data_science_and_predictive_analytics"
category: "basic_concepts"
subcategory: ""
tags: ["analytics", "business", "statistics"]
difficulty: 3
weight: 1
slug: "data_science_and_predictive_analytics"
aliases:
  - /cs/terms/data_science_and_predictive_analytics/
date: "2026-07-18T15:50:21.142766Z"
lastmod: "2026-07-18T17:15:09.115881Z"
draft: false
source: "agnes_llm"
status: "published"
language: "cs"
description: "Toto obor kombinuje statistickou analýzu a strojové učení k extrakci poznatků z dat a předpovídání budoucích událostí nebo chování."
---

## Definition

Data science zahrnuje interdisciplinární proces extrakce znalostí ze strukturovaných i nestrukturovaných dat, zatímco prediktivní analytika se konkrétně zaměřuje na používání historických dat k předpovídání budoucnosti

### Summary

Toto obor kombinuje statistickou analýzu a strojové učení k extrakci poznatků z dat a předpovídání budoucích událostí nebo chování.

## Key Concepts

- Statistické modelování
- Predikce
- Rozpoznávání vzorů
- Podpora rozhodování

## Use Cases

- Predikce odchozích zákazníků
- Posuzování finančních rizik
- Predikce poptávky v dodavatelském řetězci

## Related Terms

- [Strojové učení (Machine Learning)](/en/terms/strojové-učení-machine-learning/)
- [Byznys analytika (Business Intelligence)](/en/terms/byznys-analytika-business-intelligence/)
- [Statistika (Statistics)](/en/terms/statistika-statistics/)
- [Velká data (Big Data)](/en/terms/velká-data-big-data/)
