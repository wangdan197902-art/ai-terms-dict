---
title: אינטליגנציה כללית מלאכותית
term_id: artificial_general_intelligence
category: basic_concepts
subcategory: ''
tags:
- Future Tech
- theory
- intelligence
difficulty: 5
weight: 1
slug: artificial_general_intelligence
date: '2026-07-18T15:42:30.696334Z'
lastmod: '2026-07-18T17:15:09.512994Z'
draft: false
source: agnes_llm
status: published
language: he
description: בינה מלאכותית היפותטית בעלת היכולת להבין, ללמוד ולהחיל ידע במגוון רחב
  של משימות ברמה אנושית.
---
## Definition

אינטליגנציה כללית מלאכותית (AGI) מתייחסת לסוג של בינה מלאכותית שיכולה לבצע כל משימה אינטלקטואלית שאדם יכול לבצע. בניגוד לבינה מלאכותית צרה, המתמחה במשימות ספציפיות, AGI מסוגלת להעביר ידע בין תחומים שונים ולחשוב בצורה כללית.

### Summary

בינה מלאכותית היפותטית בעלת היכולת להבין, ללמוד ולהחיל ידע במגוון רחב של משימות ברמה אנושית.

## Key Concepts

- אינטליגנציה ברמה אנושית
- למידה העברה (Transfer Learning)
- היגיון יומיומי (Common Sense Reasoning)
- הסתגלות אוטונומית

## Use Cases

- עוזרים אישיים אוניברסליים המסוגלים לתכנון מורכב
- גילוי מדעי במגוון דיסציפלינות
- מערכות חינוך מותאמות אישית המסתגלות לצרכים ספציפיים

## Related Terms

- [narrow_ai (בינה מלאכותית צרה)](/en/terms/narrow_ai-בינה-מלאכותית-צרה/)
- [superintelligence (סופר-אינטליגנציה)](/en/terms/superintelligence-סופר-אינטליגנציה/)
- [machine_learning (למידת מכונה)](/en/terms/machine_learning-למידת-מכונה/)
- [cognitive_architecture (ארכיטקטורה קוגניטיבית)](/en/terms/cognitive_architecture-ארכיטקטורה-קוגניטיבית/)
