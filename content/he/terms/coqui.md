---
title: "קוקוי"
term_id: "coqui"
category: "basic_concepts"
subcategory: ""
tags: ["speech", "open-source", "tts"]
difficulty: 2
weight: 1
slug: "coqui"
aliases:
  - /he/terms/coqui/
date: "2026-07-18T15:50:30.243377Z"
lastmod: "2026-07-18T17:15:09.523713Z"
draft: false
source: "agnes_llm"
status: "published"
language: "he"
description: "קוקוי היא חברה קוד פתוח לטכנולוגיית דיבור, המוכרת בפיתוח מודלים איכותיים רב-לשוניים להמרת טקסט לדיבור (TTS)."
---

## Definition

קוקוי טכנולוגיות הייתה שחקנית בולמת בקהילת הבינה המלאכותית בקוד פתוח, הידועה בעיקר במנוע ה-TTS שלה. הפרויקט סיפק מודלים מאומנים מראש שיכלו ליצור דיבור טבעי וברור.

### Summary

קוקוי היא חברה קוד פתוח לטכנולוגיית דיבור, המוכרת בפיתוח מודלים איכותיים רב-לשוניים להמרת טקסט לדיבור (TTS).

## Key Concepts

- המרת טקסט לדיבור
- קוד פתוח
- סינתזה רב-לשונית
- שיבוט קול

## Use Cases

- כלי נגישות למשתמשים עיוורים
- דיבוב אוטומטי לסרטונים
- מערכות תגובה קולית אינטראקטיביות

## Related Terms

- [Tacotron (מודל למידה עמוקה להמרת טקסט לדיבור)](/en/terms/tacotron-מודל-למידה-עמוקה-להמרת-טקסט-לדיבור/)
- [VITS (מודל קול רב-לשוני מבוסס גנרציה)](/en/terms/vits-מודל-קול-רב-לשוני-מבוסס-גנרציה/)
- [Speech Recognition (זיהוי דיבור)](/en/terms/speech-recognition-זיהוי-דיבור/)
- [Natural Language Processing (עיבוד שפה טבעית)](/en/terms/natural-language-processing-עיבוד-שפה-טבעית/)
