---
title: "Scor"
term_id: "score"
category: "basic_concepts"
subcategory: ""
tags: ["evaluation", "metrics"]
difficulty: 1
weight: 1
slug: "score"
aliases:
  - /ro/terms/score/
date: "2026-07-18T15:29:21.851292Z"
lastmod: "2026-07-18T17:15:09.603520Z"
draft: false
source: "agnes_llm"
status: "published"
language: "ro"
description: "Un scor este o valoare numerică care reprezintă calitatea, încrederea sau aptitudinea predicției sau soluției unui model."
---

## Definition

Scorurile cuantifică cât de bine se descurcă un model de învățare automată în raport cu metrici specifice, cum ar fi acuratețea, precizia sau recompensa. În învățarea prin întărire, scorurile indică recompensele cumulative, în timp ce în clas

### Summary

Un scor este o valoare numerică care reprezintă calitatea, încrederea sau aptitudinea predicției sau soluției unui model.

## Key Concepts

- Evaluarea Metricilor
- Nivelul de Încredere
- Semnal de Recompensă
- Ținta de Optimizare

## Use Cases

- Evaluarea acurateței clasificării
- Urmărirea progresului agentului RL
- Clasarea rezultatelor căutării

## Related Terms

- [Metric (Metrică)](/en/terms/metric-metrică/)
- [Accuracy (Acuratețe)](/en/terms/accuracy-acuratețe/)
- [Reward (Recompensă)](/en/terms/reward-recompensă/)
- [Evaluation (Evaluare)](/en/terms/evaluation-evaluare/)
