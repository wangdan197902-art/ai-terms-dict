---
title: "Echitate"
term_id: "fairness"
category: "ethics_safety"
subcategory: ""
tags: ["ethics", "bias", "safety"]
difficulty: 3
weight: 1
slug: "fairness"
aliases:
  - /ro/terms/fairness/
date: "2026-07-18T15:35:52.633739Z"
lastmod: "2026-07-18T17:15:09.614066Z"
draft: false
source: "agnes_llm"
status: "published"
language: "ro"
description: "Echitatea se referă la principiul conform căruia sistemele de inteligență artificială ar trebui să evite generarea unor rezultate părtinitoare sau discriminatorii împotriva anumitor grupuri."
---

## Definition

În inteligența artificială, echitatea este o metrică etică critică care asigură că algoritmii nu perpetuează sau nu amplifică prejudecățile sociale bazate pe atribute protejate, cum ar fi rasa, genul sau vârsta. Aceasta implică implementarea de mecanisme pentru identificarea și atenuarea bias-urilor în datele de antrenament și în deciziile automate.

### Summary

Echitatea se referă la principiul conform căruia sistemele de inteligență artificială ar trebui să evite generarea unor rezultate părtinitoare sau discriminatorii împotriva anumitor grupuri.

## Key Concepts

- Atenuarea bias-ului
- Impact disproporționat
- Echitate
- Justiție algoritmică

## Use Cases

- Evaluarea algoritmilor de recrutare
- Sisteme de aprobare a creditelor
- Predicția recidivei criminale

## Related Terms

- [bias (părtinire/prejudecată)](/en/terms/bias-părtinire-prejudecată/)
- [ethics (etică)](/en/terms/ethics-etică/)
- [explainability (explicabilitate)](/en/terms/explainability-explicabilitate/)
- [accountability (răspundere)](/en/terms/accountability-răspundere/)
