---
title: "Dataset: Embedding Data/Paq Pairs"
term_id: "datasetembedding_datapaq_pairs"
category: "training_techniques"
subcategory: ""
tags: ["Retrieval", "QA Systems", "Datasets"]
difficulty: 4
weight: 1
slug: "datasetembedding_datapaq_pairs"
aliases:
  - /ro/terms/datasetembedding_datapaq_pairs/
date: "2026-07-18T15:52:13.995456Z"
lastmod: "2026-07-18T17:15:09.643391Z"
draft: false
source: "agnes_llm"
status: "published"
language: "ro"
description: "Un set de date la scară largă de perechi întrebare-răspuns derivate din Wikipedia, conceput pentru antrenarea recuperării pasaje dense."
---

## Definition

Setul de date PAQ (Pseudo-Answer Quality) conține milioane de perechi întrebare-răspuns generate automat, extrase din Wikipedia. Este inginerizat specific pentru antrenarea recuperatoarelor dense prin furnizarea...

### Summary

Un set de date la scară largă de perechi întrebare-răspuns derivate din Wikipedia, conceput pentru antrenarea recuperării pasaje dense.

## Key Concepts

- Recuperarea pasaje dense
- Eșantionarea negativă
- Răspuns la întrebări în domeniu deschis
- Extragere din Wikipedia

## Use Cases

- Antrenarea recuperatoarelor dense
- Răspuns la întrebări în domeniu deschis
- Evaluarea performanței de recuperare

## Related Terms

- [DPR (Dense Passage Retrieval)](/en/terms/dpr-dense-passage-retrieval/)
- [Bi-Încodator](/en/terms/bi-încodator/)
- [Bază de cunoștințe](/en/terms/bază-de-cunoștințe/)
