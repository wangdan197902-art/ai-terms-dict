---
title: "Perspective langage/action"
term_id: "languageaction_perspective"
category: "basic_concepts"
subcategory: ""
tags: ["nlp", "linguistics", "theory"]
difficulty: 4
weight: 1
slug: "languageaction_perspective"
aliases:
  - /fr/terms/languageaction_perspective/
date: "2026-07-18T11:25:14.947317Z"
lastmod: "2026-07-18T11:44:45.282897Z"
draft: false
source: "agnes_llm"
status: "published"
language: "fr"
description: "Un cadre théorique considérant le langage principalement comme une forme d'action sociale plutôt que comme un simple système de description de la réalité."
---

## Definition

Ancrée dans la théorie des actes de parole et la pragmatique, cette perspective met l'accent sur la manière dont les énoncés remplissent des fonctions telles que la demande, la promesse ou l'ordre. En Traitement Automatique du Langage Naturel (NLP), elle éclaire la détection des intentions.

### Summary

Un cadre théorique considérant le langage principalement comme une forme d'action sociale plutôt que comme un simple système de description de la réalité.

## Key Concepts

- Théorie des actes de parole
- Pragmatique
- Reconnaissance d'intention
- Systèmes de dialogue

## Use Cases

- Conception d'agents conversationnels IA
- Analyse des interactions avec le service client
- Compréhension du sens implicite dans le texte

## Related Terms

- [pragmatics (pragmatique)](/en/terms/pragmatics-pragmatique/)
- [intent_classification (classification d'intention)](/en/terms/intent_classification-classification-d-intention/)
- [dialogue_management (gestion de dialogue)](/en/terms/dialogue_management-gestion-de-dialogue/)
- [speech_act (acte de parole)](/en/terms/speech_act-acte-de-parole/)
