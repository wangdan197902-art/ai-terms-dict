---
title: "Verkörpert"
term_id: "embodied"
category: "basic_concepts"
subcategory: ""
tags: ["robotics", "interaction", "physical"]
difficulty: 3
weight: 1
slug: "embodied"
aliases:
  - /de/terms/embodied/
date: "2026-07-18T10:49:35.954651Z"
lastmod: "2026-07-18T11:44:44.872344Z"
draft: false
source: "agnes_llm"
status: "published"
language: "de"
description: "Verkörperte KI bezieht sich auf intelligente Systeme, die über Sensoren und Aktuatoren in einem Körper mit der physischen Welt interagieren."
---

## Definition

Verkörperte KI geht davon aus, dass Intelligenz aus der Interaktion zwischen der physischen Form eines Agenten und seiner Umgebung entsteht. Im Gegensatz zur entkörperlichten KI, die abstrakte Daten verarbeitet, nutzen verkörperte Agenten Sensoren, um ihre Umgebung wahrzunehmen, und Aktuatoren, um darauf zu reagieren.

### Summary

Verkörperte KI bezieht sich auf intelligente Systeme, die über Sensoren und Aktuatoren in einem Körper mit der physischen Welt interagieren.

## Key Concepts

- Sensorimotorische Schleife
- Physische Interaktion
- Robotik
- Gegründete Kognition

## Use Cases

- Autonom fahrende Fahrzeuge
- Roboter in der Montagelinie
- Humanoidroboter zur Unterstützung im Haushalt

## Related Terms

- [Robotik (Robotics)](/en/terms/robotik-robotics/)
- [Wahrnehmung (Perception)](/en/terms/wahrnehmung-perception/)
- [Aktuation (Actuation)](/en/terms/aktuation-actuation/)
- [Simulation (Simulation)](/en/terms/simulation-simulation/)
