---
title: "Claude"
term_id: "claude"
category: "basic_concepts"
subcategory: ""
tags: ["LLMs", "AI Models", "Anthropic"]
difficulty: 2
weight: 1
slug: "claude"
date: "2026-07-18T10:57:35.607319Z"
lastmod: "2026-07-18T11:44:44.893223Z"
draft: false
source: "agnes_llm"
status: "published"
language: "de"
description: "Claude ist eine Familie großer Sprachmodelle, die von Anthropic entwickelt wurde und darauf ausgelegt ist, hilfreiche, harmlose und ehrliche KI-Assistenten zu sein."
---
## Definition

Claude ist eine Reihe fortschrittlicher großer Sprachmodelle, die vom KI-Sicherheitsunternehmen Anthropic erstellt wurden. Bekannt für seine starken Prinzipien der Ausrichtung und den Rahmen der verfassungsmäßigen KI konzentriert sich Claude darauf, hilfreich

### Summary

Claude ist eine Familie großer Sprachmodelle, die von Anthropic entwickelt wurde und darauf ausgelegt ist, hilfreiche, harmlose und ehrliche KI-Assistenten zu sein.

## Key Concepts

- Anthropic
- Verfassungsmäßige KI
- Großes Sprachmodell
- KI-Ausrichtung

## Use Cases

- Unternehmensdokumentenanalyse
- Codegenerierung und -überprüfung
- Sichere Konversationsagenten

## Related Terms

- [GPT (Modellfamilie von OpenAI)](/en/terms/gpt-modellfamilie-von-openai/)
- [LLaMA (offenes Sprachmodell von Meta)](/en/terms/llama-offenes-sprachmodell-von-meta/)
- [Anthropic (Entwickler von Claude)](/en/terms/anthropic-entwickler-von-claude/)
- [Verfassungsmäßige KI (Ansatz zur KI-Sicherheit)](/en/terms/verfassungsmäßige-ki-ansatz-zur-ki-sicherheit/)
