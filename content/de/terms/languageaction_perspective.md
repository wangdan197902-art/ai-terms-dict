---
title: Sprach-/Handlungsperspektive
term_id: languageaction_perspective
category: basic_concepts
subcategory: ''
tags:
- NLP
- linguistics
- theory
difficulty: 4
weight: 1
slug: languageaction_perspective
date: '2026-07-18T11:21:06.047159Z'
lastmod: '2026-07-18T11:44:44.957092Z'
draft: false
source: agnes_llm
status: published
language: de
description: Ein theoretischer Rahmen, der Sprache primär als Form sozialen Handelns
  und nicht nur als System zur Beschreibung der Realität betrachtet.
---
## Definition

Verankert in der Sprechakttheorie und Pragmatik, betont diese Perspektive, wie Äußerungen Funktionen wie Anfordern, Versprechen oder Befehlen erfüllen. Im Natural Language Processing (NLP) informiert sie die Entwicklung von Systemen, die Absichten verstehen.

### Summary

Ein theoretischer Rahmen, der Sprache primär als Form sozialen Handelns und nicht nur als System zur Beschreibung der Realität betrachtet.

## Key Concepts

- Sprechakttheorie
- Pragmatik
- Absichtserkennung
- Dialogsysteme

## Use Cases

- Design von conversational AI Agents (konversationale KI-Agenten)
- Analyse von Kundenservice-Interaktionen
- Verstehen impliziter Bedeutungen in Texten

## Related Terms

- [pragmatics (Pragmatik)](/en/terms/pragmatics-pragmatik/)
- [intent_classification (Absichtsklassifizierung)](/en/terms/intent_classification-absichtsklassifizierung/)
- [dialogue_management (Dialogmanagement)](/en/terms/dialogue_management-dialogmanagement/)
- [speech_act (Sprechakt)](/en/terms/speech_act-sprechakt/)
