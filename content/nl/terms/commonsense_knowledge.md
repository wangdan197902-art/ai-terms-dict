---
title: "Algemene kennis"
term_id: "commonsense_knowledge"
category: "basic_concepts"
subcategory: ""
tags: ["knowledge_rep", "reasoning", "nlp"]
difficulty: 3
weight: 1
slug: "commonsense_knowledge"
aliases:
  - /nl/terms/commonsense_knowledge/
date: "2026-07-18T15:46:49.096449Z"
lastmod: "2026-07-18T17:15:08.726622Z"
draft: false
source: "agnes_llm"
status: "published"
language: "nl"
description: "De achtergrondinformatie en intuïtieve begrip van de fysieke en sociale wereld die mensen bezitten, maar machines vaak missen."
---

## Definition

Algemene kennis verwijst naar de enorme hoeveelheid impliciete informatie over het dagelijks leven, natuurkunde, sociale normen en oorzaak-gevolgrelaties die mensen natuurlijk verwerven. In de AI is het verkrijgen van deze kennis een grote uitdaging, omdat machines vaak moeite hebben met het begrijpen van context en subtiele menselijke interpretaties.

### Summary

De achtergrondinformatie en intuïtieve begrip van de fysieke en sociale wereld die mensen bezitten, maar machines vaak missen.

## Key Concepts

- Impliciete kennis
- Redeneren
- Contextbegrip
- Menselijke intuïtie

## Use Cases

- Ontwarring in natuurlijke taalverwerking (NLP)
- Navigatie en interactie in robotica
- Vraag-en-antwoopsystemen

## Related Terms

- [Kennisgrafieken (gestructureerde weergave van relaties tussen entiteiten)](/en/terms/kennisgrafieken-gestructureerde-weergave-van-relaties-tussen-entiteiten/)
- [Redeneren (het proces van logisch afleiden)](/en/terms/redeneren-het-proces-van-logisch-afleiden/)
- [NLP (natuurlijke taalverwerking)](/en/terms/nlp-natuurlijke-taalverwerking/)
- [Belichaamde AI (AI die fysiek in de wereld opereert)](/en/terms/belichaamde-ai-ai-die-fysiek-in-de-wereld-opereert/)
