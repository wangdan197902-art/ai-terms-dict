---
title: โคกี (Coqui)
term_id: coqui
category: basic_concepts
subcategory: ''
tags:
- speech
- Open Source
- tts
difficulty: 2
weight: 1
slug: coqui
date: '2026-07-18T15:47:03.838869Z'
lastmod: '2026-07-18T16:38:07.589883Z'
draft: false
source: agnes_llm
status: published
language: th
description: โคกีคือบริษัทเทคโนโลยีเสียงแบบโอเพนซอร์สที่มีชื่อเสียงในการพัฒนาโมเดลแปลงข้อความเป็นเสียง
  (TTS) คุณภาพสูงและรองรับหลายภาษา
---
## Definition

โคกี เทคโนโลยีส์ เป็นผู้เล่นสำคัญในชุมชนปัญญาประดิษฐ์แบบโอเพนซอร์ส โดยมีชื่อเสียงที่สุดจากเครื่องยนต์แปลงข้อความเป็นเสียง (TTS) ของโครงการ ซึ่งนำเสนอโมเดลที่ผ่านการฝึกฝนมาแล้ว สามารถสร้างเสียงพูดที่ฟังดูเป็นธรรมชาติได้

### Summary

โคกีคือบริษัทเทคโนโลยีเสียงแบบโอเพนซอร์สที่มีชื่อเสียงในการพัฒนาโมเดลแปลงข้อความเป็นเสียง (TTS) คุณภาพสูงและรองรับหลายภาษา

## Key Concepts

- การแปลงข้อความเป็นเสียง (Text-to-Speech)
- โอเพนซอร์ส (Open Source)
- การสังเคราะห์เสียงหลายภาษา (Multilingual Synthesis)
- การจำลองเสียง (Voice Cloning)

## Use Cases

- เครื่องมือช่วยเข้าถึงข้อมูลสำหรับผู้พิการทางสายตา
- การสร้างเสียงบรรยายอัตโนมัติสำหรับวิดีโอ
- ระบบตอบรับด้วยเสียงอัตโนมัติ (IVR)

## Related Terms

- [Tacotron (สถาปัตยกรรมโมเดล TTS)](/en/terms/tacotron-สถาป-ตยกรรมโมเดล-tts/)
- [VITS (โมเดล TTS แบบ end-to-end)](/en/terms/vits-โมเดล-tts-แบบ-end-to-end/)
- [Speech Recognition (การรู้จำเสียงพูด)](/en/terms/speech-recognition-การร-จำเส-ยงพ-ด/)
- [Natural Language Processing (การประมวลผลภาษาธรรมชาติ)](/en/terms/natural-language-processing-การประมวลผลภาษาธรรมชาต/)
