---
title: ChatGLM
term_id: chatglm
category: basic_concepts
subcategory: ''
tags:
- models
- Chinese NLP
- Open Source
difficulty: 3
weight: 1
slug: chatglm
date: '2026-07-18T15:45:07.274922Z'
lastmod: '2026-07-18T16:38:07.583065Z'
draft: false
source: agnes_llm
status: published
language: th
description: ChatGLM เป็นซีรีส์ของโมเดลภาษาโอเพนซอร์สขนาดใหญ่และสองภาษา ที่พัฒนาโดย
  Zhipu AI และปรับให้เหมาะสมสำหรับการสนทนาภาษาจีนและอังกฤษ
---
## Definition

ChatGLM เป็นตระกูลของโมเดลภาษาที่อาศัยสถาปัตยกรรม Transformer ซึ่งออกแบบมาโดยเฉพาะเพื่อจัดการกับการสนทนาสองภาษา (จีน-อังกฤษ) คุณภาพสูง พัฒนาโดย Zhipu AI โมเดลเหล่านี้ใช้ประโยชน์จาก...

### Summary

ChatGLM เป็นซีรีส์ของโมเดลภาษาโอเพนซอร์สขนาดใหญ่และสองภาษา ที่พัฒนาโดย Zhipu AI และปรับให้เหมาะสมสำหรับการสนทนาภาษาจีนและอังกฤษ

## Key Concepts

- โมเดลสองภาษา (Bilingual Model)
- โอเพนซอร์ส (Open Source)
- ประสิทธิภาพพารามิเตอร์ (Parameter Efficiency)
- การปรับแต่งด้วยคำแนะนำ (Instruction Tuning)

## Use Cases

- บริการแปลภาษาจีน-อังกฤษ
- การติดตั้งผู้ช่วย AI ในระบบท้องถิ่น (Local Deployment)
- การวิจัยเกี่ยวกับสถาปัตยกรรม LLM ที่มีประสิทธิภาพ

## Related Terms

- [LLM (Large Language Model)](/en/terms/llm-large-language-model/)
- [Zhipu AI (บริษัทผู้พัฒนา)](/en/terms/zhipu-ai-บร-ษ-ทผ-พ-ฒนา/)
- [Transformer (สถาปัตยกรรมทรานส์ฟอร์เมอร์)](/en/terms/transformer-สถาป-ตยกรรมทรานส-ฟอร-เมอร/)
- [P-Tuning (เทคนิคการปรับแต่งพารามิเตอร์บางส่วน)](/en/terms/p-tuning-เทคน-คการปร-บแต-งพาราม-เตอร-บางส-วน/)
