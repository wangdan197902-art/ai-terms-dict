---
title: Uskomus-halu-tarkoitus-malli
term_id: beliefdesireintention_model
category: basic_concepts
subcategory: ''
tags:
- Agent Architecture
- Cognitive Science
- autonomy
difficulty: 3
weight: 1
slug: beliefdesireintention_model
date: '2026-07-18T15:45:35.304627Z'
lastmod: '2026-07-18T17:15:09.388540Z'
draft: false
source: agnes_llm
status: published
language: fi
description: Filosofinen viitekehote tekoälyssä, jota käytetään rationaalisten agenttien
  mallintamiseen heidän uskomuksiensa, halujensa ja tarkoitustensa perusteella.
---
## Definition

Uskomus-Halu-Tarkoitus (BDI)-malli on kognitiivinen arkkitehturi itsenäisten agenttien suunnitteluun, jotka tekevät rationaalisia päätöksiä. Uskomukset edustavat agentin tietoa maailmasta, halut ovat sen tavoitteita ja tarkoitus on valittuja toimintasuunnitelmia näiden tavoitteiden saavuttamiseksi.

### Summary

Filosofinen viitekehote tekoälyssä, jota käytetään rationaalisten agenttien mallintamiseen heidän uskomuksiensa, halujensa ja tarkoitustensa perusteella.

## Key Concepts

- Rationaalinen toimija
- Kognitiivinen arkkitehtuuri
- Tavokeohjautunut käyttäytyminen
- Suunnitelman suoritus

## Use Cases

- Moniagenttijärjestelmät
- Itseohjautuva robotiikka
- Älykodin automaatio

## Related Terms

- [Agent-oriented programming (Agenttilähtoinen ohjelmointi)](/en/terms/agent-oriented-programming-agenttilähtoinen-ohjelmointi/)
- [GOAL language (GOAL-kieli)](/en/terms/goal-language-goal-kieli/)
- [Rational choice theory (Rationaalisen valinnan teoria)](/en/terms/rational-choice-theory-rationaalisen-valinnan-teoria/)
- [Cognitive modeling (Kognitiivinen mallinnus)](/en/terms/cognitive-modeling-kognitiivinen-mallinnus/)
