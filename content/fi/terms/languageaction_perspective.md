---
title: "Kielen/toiminnan näkökulma"
term_id: "languageaction_perspective"
category: "basic_concepts"
subcategory: ""
tags: ["nlp", "linguistics", "theory"]
difficulty: 4
weight: 1
slug: "languageaction_perspective"
aliases:
  - /fi/terms/languageaction_perspective/
date: "2026-07-18T16:06:12.544254Z"
lastmod: "2026-07-18T17:15:09.426956Z"
draft: false
source: "agnes_llm"
status: "published"
language: "fi"
description: "Teoreettinen viitekehys, joka tarkastelee kieltä ensisijaisesti sosiaalisen toiminnan muotona eikä vain todellisuuden kuvausjärjestelmänä."
---

## Definition

Tämä näkökulma perustuu puhetapausteoriaan ja pragmaattiseen lingvistiikkaan, ja se korostaa, miten lauseet suorittavat toimintoja kuten pyyntöjä, lupauksia tai käskyjä. Tekstinkäsittelyssä se ohjaa ymmärrystä puheaktien merkityksestä.

### Summary

Teoreettinen viitekehys, joka tarkastelee kieltä ensisijaisesti sosiaalisen toiminnan muotona eikä vain todellisuuden kuvausjärjestelmänä.

## Key Concepts

- Puhetapausteoria
- Pragmaattinen lingvistiikka
- Tarkoituksen tunnistus
- Keskustelujärjestelmät

## Use Cases

- Konversioälyagenttien suunnittelu
- Asiakaspalveluviestinnän analysointi
- Tekstin implisiittisen merkityksen ymmärtäminen

## Related Terms

- [pragmatics (pragmaattinen lingvistiikka)](/en/terms/pragmatics-pragmaattinen-lingvistiikka/)
- [intent_classification (tarkoituksen luokittelu)](/en/terms/intent_classification-tarkoituksen-luokittelu/)
- [dialogue_management (keskustelun hallinta)](/en/terms/dialogue_management-keskustelun-hallinta/)
- [speech_act (puhetapa)](/en/terms/speech_act-puhetapa/)
