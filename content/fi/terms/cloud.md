---
title: "Pilvi"
term_id: "cloud"
category: "basic_concepts"
subcategory: ""
tags: ["infrastructure", "deployment", "computing"]
difficulty: 1
weight: 1
slug: "cloud"
date: "2026-07-18T15:24:20.048396Z"
lastmod: "2026-07-18T17:15:09.347416Z"
draft: false
source: "agnes_llm"
status: "published"
language: "fi"
description: "Pilvi viittaa internetissä sijaitseviin etäpalvelimiin, joita käytetään datan ja tekoälymallien tallentamiseen, hallintaan ja käsittelyyn paikallisen laitteiston sijaan."
---
## Definition

Pilvipalvelut tarjoavat skaalautuvan infrastruktuurin tekoälytyökuormille, mahdollistaen kehittäjille pääsyn tehokkaisiin GPU-resursseihin ja tallennustilaan ilman fyysisiä datakeskuksia. Pilvi tukee erilaisia palvelumalleja kuten IaaS, PaaS ja SaaS.

### Summary

Pilvi viittaa internetissä sijaitseviin etäpalvelimiin, joita käytetään datan ja tekoälymallien tallentamiseen, hallintaan ja käsittelyyn paikallisen laitteiston sijaan.

## Key Concepts

- Skaalautuvuus
- Infrastruktuuri palveluna (IaaS)
- Etätallennus
- Joustavuus (Elastisuus)

## Use Cases

- Suurten kielimallien koulutus
- Tekeäly-API-palveluiden isännöinti verkkosovelluksille
- Valtavien tietojoukkojen tallentaminen analytiikkaa varten

## Related Terms

- [Palveluntarvepohjainen laskenta (Serverless)](/en/terms/palveluntarvepohjainen-laskenta-serverless/)
- [Reunalaskenta (Edge Computing)](/en/terms/reunalaskenta-edge-computing/)
- [Datakeskus](/en/terms/datakeskus/)
- [Virtuaalisointi](/en/terms/virtuaalisointi/)
