---
title: Aprendizado por Aprendizagem
term_id: apprenticeship_learning
category: training_techniques
subcategory: ''
tags:
- Reinforcement Learning
- imitation
- training
difficulty: 4
weight: 1
slug: apprenticeship_learning
date: '2026-07-18T14:49:49.466371Z'
lastmod: '2026-07-18T15:51:59.462895Z'
draft: false
source: agnes_llm
status: published
language: pt
description: Um método de aprendizado por reforço no qual um agente aprende uma política
  imitando demonstrações de um especialista.
---
## Definition

O aprendizado por aprendizagem, também conhecido como aprendizado por reforço inverso a partir de demonstrações, permite que os agentes adquiram habilidades observando o comportamento de especialistas, em vez de depender exclusivamente de funções de recompensa.

### Summary

Um método de aprendizado por reforço no qual um agente aprende uma política imitando demonstrações de um especialista.

## Key Concepts

- Aprendizado por Reforço Inverso
- Demonstrações de Especialistas
- Imitação de Políticas
- Inferência de Recompensa

## Use Cases

- Treinamento de direção autônoma
- Tarefas de manipulação robótica
- IA para jogos

## Related Terms

- [Imitation Learning (Aprendizado por Imitação)](/en/terms/imitation-learning-aprendizado-por-imitação/)
- [Reinforcement Learning (Aprendizado por Reforço)](/en/terms/reinforcement-learning-aprendizado-por-reforço/)
- [Demonstration-Based Learning (Aprendizado Baseado em Demonstrações)](/en/terms/demonstration-based-learning-aprendizado-baseado-em-demonstrações/)
- [Reward Shaping (Modelagem de Recompensa)](/en/terms/reward-shaping-modelagem-de-recompensa/)
