---
title: "Modelo crença-desejo-intenção"
term_id: "beliefdesireintention_model"
category: "basic_concepts"
subcategory: ""
tags: ["agent_architecture", "cognitive_science", "autonomy"]
difficulty: 3
weight: 1
slug: "beliefdesireintention_model"
aliases:
  - /pt/terms/beliefdesireintention_model/
date: "2026-07-18T14:51:42.660838Z"
lastmod: "2026-07-18T15:51:59.468431Z"
draft: false
source: "agnes_llm"
status: "published"
language: "pt"
description: "Uma estrutura filosófica usada em IA para modelar agentes racionais com base em suas crenças, desejos e intenções."
---

## Definition

O modelo Crença-Desejo-Intenção (BDI) é uma arquitetura cognitiva para projetar agentes autônomos que tomam decisões racionais. As crenças representam o conhecimento do agente sobre o mundo, os desejos são os objetivos e as intenções são os planos escolhidos.

### Summary

Uma estrutura filosófica usada em IA para modelar agentes racionais com base em suas crenças, desejos e intenções.

## Key Concepts

- Agência racional
- Arquitetura cognitiva
- Comportamento orientado a metas
- Execução de planos

## Use Cases

- Sistemas multiagente
- Robótica autônoma
- Automação residencial inteligente

## Related Terms

- [Programação orientada a agentes (paradigma de programação)](/en/terms/programação-orientada-a-agentes-paradigma-de-programação/)
- [Linguagem GOAL (linguagem para agentes BDI)](/en/terms/linguagem-goal-linguagem-para-agentes-bdi/)
- [Teoria da escolha racional (teoria econômica/sociológica)](/en/terms/teoria-da-escolha-racional-teoria-econômica-sociológica/)
- [Modelagem cognitiva (representação de processos mentais)](/en/terms/modelagem-cognitiva-representação-de-processos-mentais/)
