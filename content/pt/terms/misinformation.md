---
title: "Desinformação (não intencional)"
term_id: "misinformation"
category: "ethics_safety"
subcategory: ""
tags: ["ethics", "safety", "LLM"]
difficulty: 2
weight: 1
slug: "misinformation"
date: "2026-07-18T15:11:48.782887Z"
lastmod: "2026-07-18T15:51:59.512881Z"
draft: false
source: "agnes_llm"
status: "published"
language: "pt"
description: "Informação falsa ou imprecisa que é divulgada independentemente da intenção de enganar."
---
## Definition

Desinformação refere-se a informações falsas ou enganosas compartilhadas sem a intenção deliberada de causar dano ou enganar. Diferente da desinformação maliciosa (disinformation), que é fabricada intencionalmente. No contexto da IA, muitas vezes surge como resultado de alucinações de modelos de linguagem grandes que geram fatos plausíveis, mas incorretos.

### Summary

Informação falsa ou imprecisa que é divulgada independentemente da intenção de enganar.

## Key Concepts

- Falsidades
- Ausência de intenção
- Alucinação de IA
- Verificação de fatos

## Use Cases

- Sistemas de moderação de conteúdo
- Robôs de verificação de fatos
- Educação em alfabetização midiática

## Related Terms

- [disinformation (Desinformação maliciosa/intencional)](/en/terms/disinformation-desinformação-maliciosa-intencional/)
- [hallucination (Alucinação de IA)](/en/terms/hallucination-alucinação-de-ia/)
- [bias (Viés algorítmico)](/en/terms/bias-viés-algorítmico/)
- [truthfulness (Veracidade)](/en/terms/truthfulness-veracidade/)
