---
title: "Robô assistente social"
term_id: "socially_assistive_robot"
category: "basic_concepts"
subcategory: ""
tags: ["robotics", "healthcare", "hri"]
difficulty: 3
weight: 1
slug: "socially_assistive_robot"
aliases:
  - /pt/terms/socially_assistive_robot/
date: "2026-07-18T15:22:37.463955Z"
lastmod: "2026-07-18T15:51:59.533184Z"
draft: false
source: "agnes_llm"
status: "published"
language: "pt"
description: "Um robô projetado para interagir com humanos para melhorar seu bem-estar físico ou psicológico, sem realizar tarefas físicas diretas."
---

## Definition

Robôs Assistidos Socialmente (SARs) são um subconjunto da interação humano-robô focado em fornecer assistência por meio de meios sociais, em vez de manipulação física. Eles utilizam estratégias de não contato, como comunicação verbal, gestos e expressões faciais, para motivar, instruir ou apoiar usuários.

### Summary

Um robô projetado para interagir com humanos para melhorar seu bem-estar físico ou psicológico, sem realizar tarefas físicas diretas.

## Key Concepts

- Interação Humano-Robô
- Assistência sem Contato
- Computação Afetiva
- Reabilitação

## Use Cases

- Companhia para idosos
- Suporte à terapia do autismo
- Motivação para fisioterapia

## Related Terms

- [Robô de Cuidados (Care Robot)](/en/terms/robô-de-cuidados-care-robot/)
- [Robô Companheiro (Companion Robot)](/en/terms/robô-companheiro-companion-robot/)
- [Computação Afetiva (Affective Computing)](/en/terms/computação-afetiva-affective-computing/)
- [IHR (Interação Humano-Robô)](/en/terms/ihr-interação-humano-robô/)
