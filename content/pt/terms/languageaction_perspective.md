---
title: "Perspectiva Linguagem/Ação"
term_id: "languageaction_perspective"
category: "basic_concepts"
subcategory: ""
tags: ["nlp", "linguistics", "theory"]
difficulty: 4
weight: 1
slug: "languageaction_perspective"
aliases:
  - /pt/terms/languageaction_perspective/
date: "2026-07-18T15:07:34.362161Z"
lastmod: "2026-07-18T15:51:59.506399Z"
draft: false
source: "agnes_llm"
status: "published"
language: "pt"
description: "Uma estrutura teórica que vê a linguagem primariamente como uma forma de ação social, e não apenas como um sistema para descrever a realidade."
---

## Definition

Enraizada na teoria dos atos de fala e na pragmática, essa perspectiva enfatiza como as utterances (enunciados) desempenham funções como solicitar, prometer ou comandar. No Processamento de Linguagem Natural (PLN), isso informa o desenvolvimento de sistemas que entendem a intenção por trás do texto.

### Summary

Uma estrutura teórica que vê a linguagem primariamente como uma forma de ação social, e não apenas como um sistema para descrever a realidade.

## Key Concepts

- Teoria dos Atos de Fala
- Pragmática
- Reconhecimento de Intenção
- Sistemas de Diálogo

## Use Cases

- Projeto de agentes de IA conversacional
- Análise de interações de atendimento ao cliente
- Compreensão de significado implícito em textos

## Related Terms

- [pragmatics (Pragmática)](/en/terms/pragmatics-pragmática/)
- [intent_classification (Classificação de Intenção)](/en/terms/intent_classification-classificação-de-intenção/)
- [dialogue_management (Gestão de Diálogo)](/en/terms/dialogue_management-gestão-de-diálogo/)
- [speech_act (Atos de Fala)](/en/terms/speech_act-atos-de-fala/)
