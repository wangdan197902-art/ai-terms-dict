---
title: 'Набор данных: Bigcode/The Stack Dedup'
term_id: datasetbigcodethe_stack_dedup
category: basic_concepts
subcategory: ''
tags:
- datasets
- Code Generation
- Open Source
difficulty: 2
weight: 1
slug: datasetbigcodethe_stack_dedup
date: '2026-07-18T15:47:40.257883Z'
lastmod: '2026-07-18T16:38:07.139184Z'
draft: false
source: agnes_llm
status: published
language: ru
description: Версия набора данных The Stack без дубликатов, подготовленная сообществом
  BigCode для удаления почти идентичных фрагментов кода с целью получения более чистых
  данных для обучения.
---
## Definition

The Stack Dedup — это специализированное подмножество набора данных The Stack, представляющего собой массивный репозиторий открытого исходного кода. В нем применяются строгие методы устранения дубликатов для удаления избыточных фрагментов кода, которые могли бы исказить обучение больших языковых моделей.

### Summary

Версия набора данных The Stack без дубликатов, подготовленная сообществом BigCode для удаления почти идентичных фрагментов кода с целью получения более чистых данных для обучения.

## Key Concepts

- Устранение дубликатов
- Языковые модели для кода (Code LLMs)
- Качество данных
- The Stack

## Use Cases

- Обучение моделей генерации кода
- Оценка возможностей написания кода
- Снижение избыточности при обучении

## Related Terms

- [The Stack](/en/terms/the-stack/)
- [BigCode](/en/terms/bigcode/)
- [Поиск по коду (Code Search)](/en/terms/поиск-по-коду-code-search/)
- [Данные для обучения LLM (LLM Training Data)](/en/terms/данные-для-обучения-llm-llm-training-data/)
