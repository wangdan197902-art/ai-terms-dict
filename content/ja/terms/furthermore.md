---
title: "さらに / その上"
term_id: "furthermore"
category: "basic_concepts"
subcategory: ""
tags: ["writing", "linguistics"]
difficulty: 1
weight: 1
slug: "furthermore"
aliases:
  - /ja/terms/furthermore/
date: "2026-07-18T10:51:14.545069Z"
lastmod: "2026-07-18T11:44:45.009484Z"
draft: false
source: "agnes_llm"
status: "published"
language: "ja"
description: "技術文書において、追加情報を導入したり、以前の議論を強化するために使用される接続副詞。"
---

## Definition

技術的なアルゴリズムではありませんが、「furthermore（さらに）」はAIのドキュメントや研究論文における重要な言語的ツールです。これはアイデアをつなぎ、次の記述が前の内容に重みを与えたり、補足情報を加えたりすることを示す役割を果たします。

### Summary

技術文書において、追加情報を導入したり、以前の議論を強化するために使用される接続副詞。

## Key Concepts

- 接続表現
- 技術文書作成
- 一貫性
- 詳述

## Use Cases

- 研究論文
- ドキュメンテーション
- 技術レポート

## Related Terms

- [Additionally (さらに / 加えて)](/en/terms/additionally-さらに-加えて/)
- [Moreover (それに加えて)](/en/terms/moreover-それに加えて/)
- [Consequently (したがって / その結果)](/en/terms/consequently-したがって-その結果/)
