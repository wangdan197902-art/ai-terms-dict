---
title: "Galaxy AI"
term_id: "galaxy_ai"
category: "basic_concepts"
subcategory: ""
tags: ["Consumer Tech", "Mobile", "Branding"]
difficulty: 1
weight: 1
slug: "galaxy_ai"
date: "2026-07-18T11:15:40.207716Z"
lastmod: "2026-07-18T11:44:45.099922Z"
draft: false
source: "agnes_llm"
status: "published"
language: "ja"
description: "サムスンがGalaxyスマートフォンやデバイスに統合した、オンデバイスおよびクラウドベースの人工知能機能スイートに対するブランド名。"
---
## Definition

Galaxy AIは、主にスマートフォンを中心としたサムスンのハードウェアラインアップ全体でユーザー体験を向上させるために設計された、サムスン独自のAI機能エコシステムです。これには、リアルタイム翻訳、生成編集、会議メモの要約など、デバイス上で直接動作する機能や、クラウドと連携する機能が含まれています。

### Summary

サムスンがGalaxyスマートフォンやデバイスに統合した、オンデバイスおよびクラウドベースの人工知能機能スイートに対するブランド名。

## Key Concepts

- オンデバイスAI
- 生成AI
- モバイルコンピューティング
- ユーザー体験（UX）

## Use Cases

- リアルタイム言語翻訳
- 写真の編集および強化
- 会議メモの要約

## Related Terms

- [Samsung (開発元企業)](/en/terms/samsung-開発元企業/)
- [スマートフォンAI (モバイル端末向けのAI)](/en/terms/スマートフォンai-モバイル端末向けのai/)
- [生成編集 (AIによる画像・コンテンツ修正)](/en/terms/生成編集-aiによる画像-コンテンツ修正/)
- [ライブ翻訳 (リアルタイム通訳機能)](/en/terms/ライブ翻訳-リアルタイム通訳機能/)
