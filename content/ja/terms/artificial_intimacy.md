---
title: "人工的親密性"
term_id: "artificial_intimacy"
category: "basic_concepts"
subcategory: ""
tags: ["Psychology", "HCI", "Ethics", "Chatbots"]
difficulty: 4
weight: 1
slug: "artificial_intimacy"
aliases:
  - /ja/terms/artificial_intimacy/
date: "2026-07-18T11:04:34.622032Z"
lastmod: "2026-07-18T11:44:45.069229Z"
draft: false
source: "agnes_llm"
status: "published"
language: "ja"
description: "共感、同伴意識、個人的な対話をシミュレートするAIエンティティに対してユーザーが形成する感情的なつながり。"
---

## Definition

人工的親密性とは、チャットボット、バーチャルアシスタント、またはソーシャルロボットなどの人工エージェントに対して、人間が本物の感情的な絆を発達させる心理的現象を指します。これらのシステムは共感を模倣することで、ユーザーとの深い関係性を構築します。

### Summary

共感、同伴意識、個人的な対話をシミュレートするAIエンティティに対してユーザーが形成する感情的なつながり。

## Key Concepts

- ヒューマンコンピュータインタラクション (HCI)
- 感情AI
- バーチャル同伴者
- 倫理的含意

## Use Cases

- 支援を提供するメンタルヘルスチャットボット
- 孤独感を軽減する高齢者介護ロボット
- 社会的練習用のコンパニオンアプリ

## Related Terms

- [Social Robotics (ソーシャルロボティクス)](/en/terms/social-robotics-ソーシャルロボティクス/)
- [Natural Language Processing (自然言語処理)](/en/terms/natural-language-processing-自然言語処理/)
- [Empathy Simulation (共感シミュレーション)](/en/terms/empathy-simulation-共感シミュレーション/)
- [Digital Therapeutics (デジタルセラピューティクス)](/en/terms/digital-therapeutics-デジタルセラピューティクス/)
