---
title: "Ezt akarjuk-e mi?"
term_id: "is_this_what_we_want"
category: "basic_concepts"
subcategory: ""
tags: ["Ethics", "Policy", "Society"]
difficulty: 2
weight: 1
slug: "is_this_what_we_want"
aliases:
  - /hu/terms/is_this_what_we_want/
date: "2026-07-18T16:06:42.857170Z"
lastmod: "2026-07-18T17:15:09.798180Z"
draft: false
source: "agnes_llm"
status: "published"
language: "hu"
description: "Egy kritikus etikai keretrendszer, amelyet az AI rendszerek társadalmi értékekkel, méltányossággal és potenciális káros következményekkel szembeni felmérésére használnak."
---

## Definition

Ez a kifejezés egy döntő kérdést jelent az AI etikában és irányításban, amely arra ösztönzi a érdekelt feleket, hogy felmérjék, a telepített AI technológiák összhangban vannak-e az emberi értékekkel és a közérdekkal. Ez magában foglalja a technológiák társadalmi hatásának alapos vizsgálatát és az etikai kockázatok kezelését.

### Summary

Egy kritikus etikai keretrendszer, amelyet az AI rendszerek társadalmi értékekkel, méltányossággal és potenciális káros következményekkel szembeni felmérésére használnak.

## Key Concepts

- AI etika
- Értékigazítás (Value Alignment)
- Társadalmi hatás
- Felelős AI

## Use Cases

- AI szabályozási politikák kidolgozása
- Vállalati AI irányítási felülvizsgálatok
- Nyilvános vita az arcfelismerésről

## Related Terms

- [Algoritmikus torzítás (Algorithmic Bias)](/en/terms/algoritmikus-torzítás-algorithmic-bias/)
- [AI biztonság (AI Safety)](/en/terms/ai-biztonság-ai-safety/)
- [Etikus AI (Ethical AI)](/en/terms/etikus-ai-ethical-ai/)
- [Irányítás (Governance)](/en/terms/irányítás-governance/)
