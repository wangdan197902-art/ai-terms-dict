---
title: GGUF
term_id: gguf
category: basic_concepts
subcategory: ''
tags:
- format
- Optimization
- Local LLM
difficulty: 3
weight: 1
slug: gguf
date: '2026-07-18T16:00:39.079531Z'
lastmod: '2026-07-18T17:15:09.786337Z'
draft: false
source: agnes_llm
status: published
language: hu
description: Agger.ai által kifejlesztett fájlformátum a kvantált nagy nyelvi modellek
  hatékony tárolására és betöltésére helyi hardveren.
---
## Definition

A GGUF (GPT-Generated Unified Format) egy bináris fájlformátum, amelyet kifejezetten nagy nyelvi modellek fogyasztói kategóriájú hardveren történő futtatására terveztek. Támogatja a különböző kvantálási technikákat, lehetővé téve a modellek méretének csökkentését anélkül, hogy drasztikusan romlana a pontosság.

### Summary

Agger.ai által kifejlesztett fájlformátum a kvantált nagy nyelvi modellek hatékony tárolására és betöltésére helyi hardveren.

## Key Concepts

- Kvantálás
- Modell szerializálás
- Helyi inferencia
- Memóriaoptimalizálás

## Use Cases

- LLM-ek futtatása laptopokon vagy asztali gépeken
- Modellek telepítése erőforrás-korlátozott perkeszközökön
- Optimalizált modell súlyok megosztása a nyílt forráskódú közösségben

## Related Terms

- [LLAMA.cpp (LLAMA.cpp könyvtár)](/en/terms/llama-cpp-llama-cpp-könyvtár/)
- [Quantization (Kvantálás)](/en/terms/quantization-kvantálás/)
- [ONNX (Nyílt neurális hálózatcsere formátum)](/en/terms/onnx-nyílt-neurális-hálózatcsere-formátum/)
- [Model Weights (Modellsúlyok)](/en/terms/model-weights-modellsúlyok/)
