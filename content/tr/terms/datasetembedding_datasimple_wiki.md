---
title: "Veri Seti: Gömme Verileri/Simple Wiki"
term_id: "datasetembedding_datasimple_wiki"
category: "training_techniques"
subcategory: ""
tags: ["datasets", "embeddings", "wikipedia"]
difficulty: 3
weight: 1
slug: "datasetembedding_datasimple_wiki"
aliases:
  - /tr/terms/datasetembedding_datasimple_wiki/
date: "2026-07-18T15:47:55.679780Z"
lastmod: "2026-07-18T16:38:07.292385Z"
draft: false
source: "agnes_llm"
status: "published"
language: "tr"
description: "Metin gömme modellerini eğitmek ve değerlendirmek için kullanılan, Basit İngilizce Wikipedia'dan türetilmiş özenle hazırlanmış bir veri seti."
---

## Definition

Bu veri seti, anadili İngilizce olmayanlar için basitleştirilmiş dilbilgisi ve kelime dağarcığına sahip olan Wikipedia'nın bu versiyonu olan Basit İngilizce Wikipedia'dan çıkarılan cümlelerden ve paragraflardan oluşur. Şu amaçla hizmet eder:

### Summary

Metin gömme modellerini eğitmek ve değerlendirmek için kullanılan, Basit İngilizce Wikipedia'dan türetilmiş özenle hazırlanmış bir veri seti.

## Key Concepts

- Anlamsal Gömmeler
- Metin Basitleştirme
- Vikipedi Korpüsü
- Model Referans Değerlendirmesi

## Use Cases

- Hafif gömme modellerinin eğitimi
- Basit bağlamlarda anlamsal benzerliğin değerlendirilmesi
- Doğal Dil İşleme erişilebilirlik araçlarının iyileştirilmesi

## Related Terms

- [Sentence Transformers (Cümle Dönüştürücüler)](/en/terms/sentence-transformers-cümle-dönüştürücüler/)
- [Semantic Search (Anlamsal Arama)](/en/terms/semantic-search-anlamsal-arama/)
- [Natural Language Processing (Doğal Dil İşleme)](/en/terms/natural-language-processing-doğal-dil-i-şleme/)
