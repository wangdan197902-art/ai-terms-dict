---
title: 'Veri Seti: Bigcode/The Stack Dedup'
term_id: datasetbigcodethe_stack_dedup
category: basic_concepts
subcategory: ''
tags:
- datasets
- Code Generation
- Open Source
difficulty: 2
weight: 1
slug: datasetbigcodethe_stack_dedup
date: '2026-07-18T15:47:24.541663Z'
lastmod: '2026-07-18T16:38:07.290362Z'
draft: false
source: agnes_llm
status: published
language: tr
description: BigCode tarafından temiz bir eğitim verisi sağlamak amacıyla neredeyse
  kopya kod parçacıklarının kaldırılmasıyla düzenlenmiş, The Stack veri setinin tekrarlar
  açısından arındırılmış versiyonu.
---
## Definition

The Stack Dedup, açık kaynaklı kodlardan oluşan devasa bir depo olan The Stack'in özelleştirilmiş bir alt kümesidir. Büyük dil modellerinin yanlı öğrenmesini önlemek için gereksiz kod parçacıklarını elemek amacıyla sıkı tekrarlar açısından arındırma teknikleri uygular.

### Summary

BigCode tarafından temiz bir eğitim verisi sağlamak amacıyla neredeyse kopya kod parçacıklarının kaldırılmasıyla düzenlenmiş, The Stack veri setinin tekrarlar açısından arındırılmış versiyonu.

## Key Concepts

- Tekrarları Temizleme (Deduplication)
- Kod Dil Modelleri (Code LLMs)
- Veri Kalitesi
- The Stack

## Use Cases

- Kod üretimi modellerinin eğitimi
- Kodlama yeteneklerinin değerlendirilmesi
- Eğitimdeki gereksiz tekrarların azaltılması

## Related Terms

- [The Stack](/en/terms/the-stack/)
- [BigCode](/en/terms/bigcode/)
- [Kod Arama (Code Search)](/en/terms/kod-arama-code-search/)
- [LLM Eğitim Verisi (LLM Training Data)](/en/terms/llm-eğitim-verisi-llm-training-data/)
